...
class TestPayPalConfig extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        try {
            ...
            // Testar se o cliente está sendo configurado corretamente
            $this->info('🔍 Testando configuração do cliente...');

            try {
                $client = app('paypal');
                $this->info('✅ Cliente PayPal instanciado');

                // Verificar configuração interna do cliente
                $clientConfig = $client->getConfiguration();
                $this->info('📋 Configuração interna do cliente:');
                ...
                $this->error('❌ Erro ao verificar cliente:');
